...
export function AdminAdminAccessHelp({ adminId, adminEmail }: Props) {
  const action = useMemo(() => resetAdminPasswordAction.bind(null, adminId), [adminId]);
  ...
  return (
    <div className="space-y-3 rounded-xl border border-[var(--input-border)] bg-slate-50 p-4">
      <div className="space-y-1.5">
        <p className="text-sm font-medium text-slate-800">Вход в админку</p>
        <p className="text-sm font-semibold text-slate-900">
          Если коллега забыл пароль или пишет, что не получается войти
        </p>
        <p className="text-sm leading-relaxed text-slate-700">
          Старый пароль посмотреть нельзя — в системе он хранится только в зашифрованном виде.
          Нажмите «Выдать новый временный пароль», затем отправьте коллеге email и новый пароль
          вручную (мессенджер или почта). Любой другой администратор может сделать это.
        </p>
      </div>
      <p className="text-sm text-slate-600">
        Email для входа: <span className="font-medium text-black">{adminEmail}</span>
      </p>
      ...
      {state.newPassword ? (
        <div className="space-y-2 rounded-lg border border-[var(--input-border)] bg-white p-3">
          ...
          <p className="text-xs text-slate-500">
            Предыдущий пароль перестанет действовать. Передайте коллеге новый пароль и напомните
